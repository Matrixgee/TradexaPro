...
const Sidebar: React.FC<SidebarProps> = ({ active, setActive }) => {
  ...
  const userMenuItems: MenuItem[] = [
    ...
    { name: "Deposit", icon: <RiLuggageDepositFill />, path: "deposit" },
    { name: "Withdrawal", icon: <BiMoneyWithdraw />, path: "withdraw" },
    { name: "My Plans", icon: <PiSwap />, path: "plans" },
  ...
  return (
    <aside className={`sidebar z-30 ${active ? "active" : ""}`}>
      ...
      <div className="sidebar_inner">
        <div className="logo">
          <img src={logo} alt="Express Trading" />
          <span className="text-slate-200">TradeXaPro</span>
        </div>
